...
export class TodoService {
  ...
  waitingTodos: Signal<Todo[]> = computed(() =>
    this.todos().filter((todo) => todo.status === 'waiting')
  );
  inProgressTodos: Signal<Todo[]> = computed(() =>
    this.todos().filter((todo) => todo.status === 'in progress')
  );
  doneTodos: Signal<Todo[]> = computed(() =>
  ...
  /**
   * elle retourne la liste des in progress todos
   *
   * @returns Todo[]
   */
  ...
  changeStatus(todo: Todo, status: TodoStatus) {
    this.todos.set(
      this.todos().map((t) => (t.id === todo.id ? { ...t, status } : t))
    );
  }
